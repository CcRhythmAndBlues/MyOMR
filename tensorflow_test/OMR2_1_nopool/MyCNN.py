...
image_size = train_x.shape[1] * train_x.shape[2]  # 图片尺寸2 对应图片尺寸为100*64  正好是80的平方
...
fla = int((train_x.shape[1] * train_x.shape[2]) * depth_out2)  # 用于扁平化处理的参数经过两层卷积池化后的图像大小*第二层的卷积核个数
# 定义各卷积层和全连接层的权重变量
Weights = {"con1_w": tf.Variable(tf.random_normal([kernel_h, kernel_w, depth_in, depth_out1])),
           ...
           "fc_w1": tf.Variable(tf.random_normal([int((image_size) * depth_out2), 1024])),
...
def conv_net(x, weights, biases, dropout):
    ...
    conv1 = conv2d(x, Weights['con1_w'], bias['conv1_b'])  # 200*80*64
    # conv1 = maxpool2d(conv1, 2)  # 经过池化层1 shape：50*32*64

    # Convolutional layer 2（卷积层2）
    conv2 = conv2d(conv1, Weights['con2_w'], bias['conv2_b'])  # 200*80*128
    # conv2 = maxpool2d(conv2, 2)  # 经过池化层2 shape:25*16*128
    ...
    prediction = tf.add(tf.matmul(fc2, Weights['out']), bias['out'], name="prediction")  # 输出预测参数
    return prediction
